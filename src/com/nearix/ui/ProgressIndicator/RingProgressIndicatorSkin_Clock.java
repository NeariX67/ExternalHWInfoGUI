 ...
 */ 
package com.nearix.ui.ProgressIndicator;
 ...
 *
 */
public class RingProgressIndicatorSkin_Clock implements Skin<RingProgressIndicator_Clock> {

    private final RingProgressIndicator_Clock indicator;
    public  final Label percentLabel = new Label();
    ...
    public RingProgressIndicatorSkin_Clock(final RingProgressIndicator_Clock indicator) {
        ...
        this.indicator.progressProperty().addListener((o, oldVal, newVal) -> {
            if (newVal.intValue() >= 0) {
                ...
                fillerArc.setLength(newVal.intValue() * -(360.0/(indicator.max-indicator.min)));
            }
        });
	...
	private void setProgressLabel(int value) {
        if (value >= indicator.min) {
            percentLabel.setText(String.format("%d", value) + indicator.unit);
        }
    }
    ...
    private void initFillerArc() {
        ...
        fillerArc.setLength(indicator.getProgress() * -(360.0/(indicator.max-indicator.min)));
    }

    private void initContainer(final RingProgressIndicator_Clock indicator2) {
        container.getStylesheets().addAll(indicator2.getStylesheets());
    ...
    private void initLabel(int value) {
        ...
        percentLabel.getStyleClass().add("circleindicator-smalllabel");
    }
    ...
    @Override
    public RingProgressIndicator_Clock getSkinnable() {
        return indicator;
    }
